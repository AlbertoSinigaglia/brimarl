...
import numpy as np
from brimarl_masked.networks.acnets_reg import ActorReg, CriticReg
import tensorflow as tf
from brimarl_masked.environment.environment import BriscolaGame, BriscolaPlayer, Agent


class ACAgentQuickReg(Agent):
    ...
    def action(self, game: BriscolaGame, player: BriscolaPlayer):
        ...
        original_probs = self.policy_net(s, training=False)[0]
    ...
    def clone(self, training=False, deep=False):
        assert type(self) is ACAgentQuickReg
        new_agent = ACAgentQuickReg(
            training=training
        )
        if deep:
