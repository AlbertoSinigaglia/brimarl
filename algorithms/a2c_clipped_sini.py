...
class A2CSiniClippedAlgorithm(Algorithm):
    ...
    def learn(self, agent: ACAgent):
        ...
        for iteration in range(self.num_learning_per_epoch_actor):
            with tf.GradientTape() as a_tape:
                ...
                distance = tf.cast(selected_actions_probs < initial_probs, tf.float32) * ((initial_probs-selected_actions_probs)/initial_probs) + \
                           tf.cast(selected_actions_probs >= initial_probs, tf.float32) * ((selected_actions_probs - initial_probs)/(1-initial_probs))

                self.assert_same_shape(distance, r_theta)

                clipped_r_theta = r_theta * tf.cast(distance < self.clip_eps, tf.float32)
