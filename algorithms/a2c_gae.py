...
class A2CGAEAlgorithm(GAEAlgorithm):
    def __init__(self, num_players, discount, num_learning_per_epoch, min_samples=128, entropy_beta=0.0, epsilon=0., lambda_coefficient=0.3):
        super().__init__(discount, lambda_coefficient=lambda_coefficient)
        self.num_players = num_players
        self.discount = discount
        self.optimizer_critic = tf.optimizers.legacy.Adam(1e-3)
        self.optimizer_actor = tf.optimizers.legacy.Adam(1e-3)
    ...
    def learn(self, agent: ACAgent):
        ...
        for _ in range(self.num_learning_per_epoch):
            with tf.GradientTape() as a_tape, tf.GradientTape() as c_tape, tf.GradientTape() as debug:
                ...
                loss_actor = -flat_gae * importance_sampling_ratio
                loss_critic = tf.losses.mean_squared_error(flat_val, flat_reward_to_go)[..., None] * importance_sampling_ratio

                self.assert_same_shape(importance_sampling_ratio, (BS*T, 1))
